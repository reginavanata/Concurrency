...
public class CounterThread extends Thread{
    ...
    // main method exists in its own thread called main
    public static void main(String[] args) throws InterruptedException{
        ...
        for (int i = 0; i < threads.length; i++) {
            threads[i].start(); //thread goes to a runnable state
        }
        //put main method to sleep 1 second (1000 milliseconds) before it begins counting
        Thread.sleep(1000); //main method is in a timed waiting state
        for (int i = 41; i < 50; i++) {
